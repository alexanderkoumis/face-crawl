...
void LoadCascades(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  mod_str_g = mod_str;
    face_cascade.load(mod_str + "/face-detect/haarcascades/frontalface.xml");
  // }
}
...
void FindFace(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  ////// CPU
  // cv::Mat img_bw;
  // cv::Mat img_c = cv::imread(col_path, cv::IMREAD_GRAY);
  cv::Mat img_bw = cv::imread(col_path, cv::IMREAD_GRAYSCALE);
  ...
  v8::Handle<v8::Array> array = v8::Array::New(isolate, faces.size() * 4);
  ...
  for (int i = 0; i < (int)faces.size(); ++i) {
    ...
    array->Set(base + 0, v8::Integer::New(isolate, face.x));
    array->Set(base + 1, v8::Integer::New(isolate, face.y));
    array->Set(base + 2, v8::Integer::New(isolate, face.x + face.width));
    array->Set(base + 3, v8::Integer::New(isolate, face.y + face.height));
